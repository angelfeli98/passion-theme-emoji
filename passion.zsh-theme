...
function update_command_status() {
    ...
    if $1;
    then
        H_SEL_EMOJI=${h_emoji[RANDOM % ${#h_emoji[@]} - 1 ]};
        arrow="$H_SEL_EMOJI %{$fg_bold[red]%}❱%{$fg_bold[yellow]%}❱%{$fg_bold[green]%}❱";
    else
	S_SEL_EMOJI=${s_emoji[RANDOM % ${#s_emoji[@]} - 1 ]};
        arrow="$S_SEL_EMOJI %{$fg_bold[red]%}❱❱❱";
    fi
...
SEL_EMOJI=${e_emoji[RANDOM % ${#e_emoji[@]} - 1 ]}

# prompt
PROMPT='$me 🤖 $(real_time) $SEL_EMOJI $(directory) $(git_status)$(command_status) ';
